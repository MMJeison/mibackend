...
def searchByConceptAndOntology (request):
    ...
    if not query or query == '':
        return JsonResponse({'error': 'No se ha especificado el concepto'}, safe=False)
    neoQuery = "MATCH (article:bibo__Document)<-[:aoc__annotatesResource]-(concept:aot__ExactQualifier)-[:aoc__hasTopic]->(ontology:n4sch__Class)  MATCH (author:foaf__Person)<-[]-(:rdf__Seq)<-[:bibo__authorList]-(article)-[:dct__publisher]->(organization:foaf__Organization)  WHERE ontology.dct__source[0] ="
    if ontology and ontology != '':
        neoQuery += "'" + ontology + "'"
    for concept in concepts:
        neoQuery += " AND concept.aoc__body[0] = '" + concept + "'"
    neoQuery += " WITH collect(DISTINCT author.foaf__name[0]) AS Autores,article,collect(distinct ontology.dct__source[0]) AS Ontología,organization RETURN DISTINCT article.bibo__pmid[0] AS PMC_ID,article.bibo__doi[0] AS DOI,article.dct__title[0] AS Título,article.bibo__abstract[0] as Abstract,article.dct__description[0] as Tipo,Ontología,Autores,organization.foaf__name[0] AS Organización,article.dct__created[0] as Fecha_de_publicación LIMIT 5"
    print(neoQuery)
    recs, meta = db.cypher_query(neoQuery)
    papers = [{'pmid': rec[0],
                'doi': rec[1],
                'title': rec[2],
                'abstract': rec[3],
